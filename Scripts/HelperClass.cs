...
namespace Core.Scripts.Utils
{
	...
	{
		public static IFormatProvider formatProvider = new NumberFormatInfo() { NumberGroupSeparator = " " };
		...
		public static T GetInactiveOrCreate<T>(ICollection<T> collection, T prefab, Transform parent,
			bool activeSelf = true, bool modifyTransform = true) where T : Component
			=> GetInactiveOrCreate(
				collection,
...
//			float gray = (c.r + c.g + c.b) / 3.0f;
		...
		public static void SetGameLayerRecursive(GameObject target, int layer)
		{
			target.layer = layer;
